...
void setup() {
  ...
  if (openArrayPos <= 0) {
    for (int i = 0; i < asteroidCount; i++) {
        ...
        asteroids.add(new Asteroid(random(75, 725), random(75, 725), level, round(random(3))));
    }
  }
  
  // sound effects
  projectile = new SoundFile(this, "362423__alphatrooper18__explosion1.wav");
  asteroidExplosion = new SoundFile(this, "430978__audiopapkin__low-impact.wav"); 
}
...
boolean collisionDetection(Bullet bullet) {
  //For Collision Detection
    ...
    // Bullet from Player to AlienShip or asteroid
      ...
      for (int j = 0; j<asteroids.size(); j++) {
        ...
        //if(testX > astX && testX < astX + astMaxWidth){
        //  if(testY > astY && testY < astY + astMaxHeight){
